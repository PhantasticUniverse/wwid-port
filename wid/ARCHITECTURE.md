...
| `termination` | Thick-flanged / Unflanged open end | Reflection coefficient model. ThickFlanged (NAF) with flange correction; Unflanged (Whistle/Flute/Reed) with Levine & Schwinger model. |
...
| `simple_fipple` | Simple fipple (Whistle/Flute) | Empirical window impedance Xw/Rw, headspace as parallel branch, EmbouchureHole extraction for flutes |
| `simple_reed` | Reed mouthpiece (single/double/lip) | Linear reactance `X = alpha × 1e-3 × freq + beta`. Pressure-node boundary: TM = `[[0+iX, z₀], [1, 0]]`. Headspace is extracted but not used (matching Java parity). |
...
**Tests**: 3 unit tests (termination module) + full validation through `wid-eval` integration tests against golden Z-samples (NAF, Whistle, Flute, Reed) and evaluation results.
...
| `predicted_frequency()` | Playing frequency via Im(Z)=0 root finding (NAF, Reed) |
| `predicted_frequency_linear_v()` | Playing frequency via gain-aware fmin search (Whistle, Flute) |
| `calculate_error_vector()` | Cents deviation per fingering (dispatches by `CalculatorParams`) |
| `CalculatorParams` | Study model parameters: `NAF`, `WHISTLE`, `FLUTE`, `REED` |
...
**Tests**: 12 unit tests + 19 integration tests:
- Unit: Brent solver (5), calculator params (2), Z-samples (1), evaluation (1), error vector (1), cents utility (2)
- Integration: bulk NAF eval (4, 36 combos, 540 fingerings), bulk Whistle eval (4, 16 combos, 272 fingerings), bulk Flute eval (4, 8 combos, 110 fingerings), Whistle Z-sample (1), Flute Z-sample (1), bulk Reed eval (4, 7 combos, 72 fingerings), Reed Z-sample (1)

---

### wid-optimize

**Role**: Calibration and optimization for instrument design across all study models.

| Type / Function | Description |
|----------------|-------------|
| `calibrate_fipple()` | 1D fipple factor calibration using Brent (NAF) |
| `window_height` | 1D window height calibration using Brent (Whistle) |
| `beta` | 1D beta calibration using Brent (Whistle/Flute) |
| `whistle_calib` | 2D joint window-height + beta calibration using BOBYQA (Whistle) |
| `airstream_length` | 1D airstream length calibration using Brent (Flute) |
| `flute_calib` | 2D joint airstream-length + beta calibration using BOBYQA (Flute) |
| `hole_size` | N-dim hole diameter optimization using BOBYQA (Whistle/Flute) |
| `hole_position` | (N+1)-dim hole position optimization using BOBYQA (Whistle/Flute) |
| `hole_combined` | (2N+1)-dim merged hole optimization using BOBYQA (Whistle/Flute) |
| `optimize_holes()` | Multi-variable hole geometry optimization using BOBYQA (NAF) |
| `global_optimize` | Two-stage DIRECT-C → BOBYQA pipeline for global hole optimization |
| `multi_start` | Multi-start BOBYQA with random/grid start points (xoshiro256** PRNG) |
...
| `FluteCalibrationResult` | Initial/final airstream length, beta, and norm |
...
**Dependencies**: `bobyqa`, `direct`, `wid-eval`, `wid-compile`, `wid-physics`, `wid-types`

**Tests**: 68 tests — Brent minimizer (4), fipple calibration (6), NAF hole optimization (7), norm calculation (3), Whistle calibrators (window height 3, beta 4, joint 2), Flute calibrators (airstream length 5, joint 3), Whistle hole optimizers (size 2, position 3, combined 3), Flute hole optimizers (size 3, position 3, combined 3), fife smoke tests (3), Reed calibrator (2), multi-start (7), global optimize (3).
...
**Role**: Standalone pure-Rust implementation of the DIRECT-C global optimizer (DIviding RECTangles, Centred variant). Zero dependencies, suitable for WASM.

Clean-room implementation from algorithm papers (Jones 1993, Gablonsky 2001) with DIRECT-C extensions matching WIDesigner's variant selection strategies. Three algorithm layers:
1. **Base DIRECT**: BTreeMap-sorted hyperrectangles, convex hull POH selection, trisection
2. **DIRECT-1**: Single-side division for non-hypercubes, per-dimension potential tracking
3. **DIRECT-C**: Alternate POH strategies (Large & Near, Low Value & Near) triggered on stagnation

**Public API**: `direct_minimize(f, lower_bounds, upper_bounds, convergence, max_evals, target_value) → Option<DirectResult>`

**Tests**: 19 unit tests + 1 doc test — Rosenbrock (2D, 5D), six-hump camel, Rastrigin, Styblinski-Tang, Goldstein-Price, sphere (1D/2D/5D), target value early stopping, callback cancellation, edge cases.
