...
**Tests**: 9 tests (parsing oracle XMLs, namespace stripping, unit conversion).

---

### wid-compile

**Role**: Convert `InstrumentRaw` → `InstrumentCompiled` — the explicit compile step that prevents "forgot to call updateComponents" bugs.
...
**Tests**: 17 tests (component count, ordering, interpolation, validation). Component count (13) and headspace sections (1) match golden values for the 6-hole NAF.
...
**Tests**: 8 tests (Z-samples against golden, evaluation within 0.5 cents for all 15 fingerings, error vector, cents utility).
