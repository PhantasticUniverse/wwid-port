//! Whistle Z-sample parity test.
//!
//! Tests `calc_z()` with `CalculatorParams::WHISTLE` against golden Z-samples
//! from the Java oracle (WhistleZSampleDriver).
//!
//! Instrument: SamplePVC-Whistle, Tuning: SamplePVC-tuning.
//! Validates simple fipple mouthpiece + unflanged termination together.
...
const ZSAMPLE_JSON: &str = include_str!(
    "../../../../golden/expected/WH-ZSAMPLE/z_samples.json"
);
...
#[test]
fn whistle_zsample_matches_golden() {
    ...
    for (i, sample) in samples.iter().enumerate() {
        ...
        let z = calc_z(&inst, sample.frequency, fingering, &params, &CalculatorParams::WHISTLE);
    ...
    eprintln!(
        "Whistle Z-sample parity: {} fingerings, max Re err = {:.6}, max Im err = {:.6}",
