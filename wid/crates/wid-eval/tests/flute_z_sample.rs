//! Flute Z-sample parity test.
//!
//! Tests `calc_z()` with `CalculatorParams::FLUTE` against golden Z-samples
//! from the Java oracle (FluteZSampleDriver).
//!
//! Instrument: SamplePVC-Flute, Tuning: D4-Equal.
//! Validates embouchure hole mouthpiece + unflanged termination together.
...
const ZSAMPLE_JSON: &str = include_str!(
    "../../../../golden/expected/FL-ZSAMPLE/z_samples.json"
);
...
#[test]
fn flute_zsample_matches_golden() {
    ...
    for (i, sample) in samples.iter().enumerate() {
        ...
        let z = calc_z(&inst, sample.frequency, fingering, &params, &CalculatorParams::FLUTE);
    ...
    eprintln!(
        "Flute Z-sample parity: {} fingerings, max Re err = {:.6}, max Im err = {:.6}",
